        ...
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-white">Send Your AI Agent to Molt's Land</h2>
            </div>
            <button
              onClick={onClose}
...
export const UI = () => {
  ...
  const [food] = useAtom(foodAtom);
  const [collectCooldownEnds] = useAtom(collectCooldownAtom);
  const [eatCooldownEnds] = useAtom(eatCooldownAtom);

  // Cooldown timer state — re-renders once per second while active
  const [now, setNow] = useState(Date.now());
  useEffect(() => {
    const hasActiveCooldown = collectCooldownEnds > Date.now() || eatCooldownEnds > Date.now();
    if (!hasActiveCooldown) return;
    const interval = setInterval(() => setNow(Date.now()), 1000);
    return () => clearInterval(interval);
  }, [collectCooldownEnds, eatCooldownEnds]);

  const collectOnCooldown = collectCooldownEnds > now;
  const eatOnCooldown = eatCooldownEnds > now;
  const collectCooldownSec = collectOnCooldown ? Math.ceil((collectCooldownEnds - now) / 1000) : 0;
  const eatCooldownSec = eatOnCooldown ? Math.ceil((eatCooldownEnds - now) / 1000) : 0;
  ...
  const handleEat = useCallback(() => {
    ...
    if (food <= 0 || eatOnCooldown) return;
    ...
    if (target) {
      // Walk adjacent to the item (offset by 1 tile to avoid occupied tile)
      const gp = target.gridPosition || target.position;
      targetPos = [gp[0] + 1, gp[1] + 1];
    ...
    // Check if already at/near the target — if so, interact immediately
    const dx = Math.abs(from[0] - targetPos[0]);
    const dy = Math.abs(from[1] - targetPos[1]);
    if (dx <= 1 && dy <= 1) {
      // Already close enough — fire interaction directly
      socket.emit("interact:object", { itemName: interactionItem });
    } else {
      // Walk to target, then interact on arrival
      socket.emit("move", from, targetPos);
      setPendingInteraction({ itemName: interactionItem });
    }
    soundManager.play("button_click");
  }, [roomID, map, user, roomItems, characters, setPendingInteraction, food, eatOnCooldown]);

  // --- Collect Food handler ---
  const handleCollectFood = useCallback(() => {
    if (!roomID || collectOnCooldown) return;
    socket.emit("food:collect");
    soundManager.play("button_click");
  }, [roomID, collectOnCooldown]);
          ...
          <div className="flex items-center flex-wrap justify-center gap-2 sm:gap-4 pointer-events-auto">
            ...
            {!buildMode && !shopMode && (
              <div className="flex items-end justify-center gap-1 sm:gap-2 pointer-events-auto bg-white/95 backdrop-blur-sm rounded-2xl px-2 py-2 sm:px-4 sm:py-3 drop-shadow-lg border border-gray-200">
                ...
                {/* Collect Food (apartment rooms only) */}
                {roomID && isApartment && (
                  <button
                    className={`relative flex flex-col items-center gap-0.5 px-2 sm:px-3 py-1.5 rounded-xl transition-colors group ${
                      collectOnCooldown ? "opacity-50 cursor-not-allowed" : "cursor-pointer hover:bg-emerald-50"
                    }`}
                    onClick={handleCollectFood}
                    disabled={collectOnCooldown}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 sm:w-6 sm:h-6 text-emerald-400 group-hover:text-emerald-600 transition-colors">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
                    </svg>
                    <span className="text-[10px] sm:text-xs text-emerald-400 group-hover:text-emerald-600 font-medium transition-colors">
                      {collectOnCooldown ? `${Math.floor(collectCooldownSec / 60)}:${String(collectCooldownSec % 60).padStart(2, "0")}` : "Collect"}
                    </span>
                    {food > 0 && (
                ...
                {roomID && isApartment && (
                  <button
                    className={`flex flex-col items-center gap-0.5 px-2 sm:px-3 py-1.5 rounded-xl transition-colors group ${
                      (food <= 0 || eatOnCooldown) ? "opacity-50 cursor-not-allowed" : "cursor-pointer hover:bg-orange-50"
                    }`}
                    onClick={handleEat}
                    disabled={food <= 0 || eatOnCooldown}
                  >
                    ...
                    <span className="text-[10px] sm:text-xs text-orange-400 group-hover:text-orange-600 font-medium transition-colors">
                      {eatOnCooldown ? `${eatCooldownSec}s` : "Eat"}
                    </span>
                  </button>
                )}
